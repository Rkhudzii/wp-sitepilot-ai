...
if ( ! empty( $address ) ) {
    $address = implode( ', ', array_filter( array( $city, $address ) ) );
} else {
    $address = implode( ', ', array_filter( array( $district, $city ) ) );
}
...
$deal_label = isset( $deal_labels[ $deal ] ) ? $deal_labels[ $deal ] : 'Обʼєкт';
...
<a href="<?php echo esc_url( $link ); ?>" class="recrm-card">
    ...
    <div class="recrm-content">
        <h3 class="recrm-card-title"><?php echo esc_html( $title ); ?></h3>
